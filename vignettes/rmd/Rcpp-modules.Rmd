...
If improperly configured this may result in an error message such as
when package installation is attempted:

```
** testing if installed package can be loaded from temporary location
Loading required package: grDevices
Error: package or namespace load failed for ‘testmod’ in .doLoadActions(where, attach):
 error in load action .__A__.1 for package testmod: loadModule(module = "yada", what = TRUE, env = ns, loadNow = TRUE): Unable to load module "yada": Failed to initialize module pointer: Error in FUN(X[[i]], ...): no such symbol _rcpp_module_boot_yada in package testmod
Error: loading failed
```

To fix this error the package author needs to compile attributes to
generate module bindings in testmod/src/RcppExports.cpp via

```{r, eval=FALSE}
Rcpp::compileAttributes("testmod")
```

If the RcppExports.cpp is not generated by compileAttributes that
typically means that there is already a `R_init_PACKAGE` call
somewhere else in your C++ code. After compiling attributes the
RcppExports.cpp code should have the following (which will fix the
error above).
