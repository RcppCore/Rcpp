...
\end{minipage}
  ...
  \end{minipage}
  \begin{minipage}[t]{0.465\linewidth}
    \centering{\underline{Sugar: Using the \R API}}
...
\bigskip
  ...
  \caption{\pkg{Rcpp} versus the \R API: Five ways of calling
  ...
  \texttt{using namespace Rcpp;} in the code. The versions that use callbacks to R do not require handling
...
Finally, we also show a variant using `Rcpp sugar', a topic which is
...
Extending \R with compiled code requires a mechanism for reliably compiling,
...
links and loads a \proglang{C}, \Cpp or \proglang{Fortran} function---directly from the \R prompt
...
updated just like any other \R package using, for examples, the
\code{install.packages()} function for initial installation as well as
\code{update.packages()} for upgrades.  So even though \R / \Cpp interfacing
...
directive in the DESCRIPTION file lets \R properly reference the header files.
...
\pkg{inline} as shown below.  The function body is provided by the \R character
...
new \R function \code{fun} based on the \Cpp code in \code{src}:
...
With one assignment to the \R variable \code{src}, and one call of the \R function
\code{cxxfunction} (provided by the \pkg{inline} package), we have created a new \R
function \code{fun} that uses the \proglang{C++} code we assigned to
\code{src}---and all this functionality can be used directly from the \R
prompt making prototyping with \Cpp functions straightforward.

\section{Using Standard Template Library algorithms}

The \Cpp Standard Template Library (STL) offers a variety of generic
...
Code that uses both \R and \Cpp has to deal with two distinct
error handling models. \pkg{Rcpp} simplifies this and allows both
systems to work together.

\subsection[C++ exceptions in R]{\Cpp exceptions in \proglang{R}}

The internals of the \R condition mechanism and the implementation of
\Cpp exceptions are both based on a layer above POSIX jumps. These layers
...
so that \Cpp exceptions are caught and recycled into the \R condition
mechanism.

\pkg{Rcpp} defines the \code{BEGIN\_RCPP} and \code{END\_RCPP} macros that should
be used to bracket code that might throw \Cpp exceptions.
...
versions---guarantees that the stack is first unwound in terms of \Cpp
exceptions, before the problem is converted to the standard \R error
management system using the function \code{Rf\_error} of the \R API.

The \code{forward\_exception\_to\_r} function uses run-time type information to
extract information about the class of the \Cpp exception and its message so that
...
handlers. \R calling handlers are also based on POSIX jumps, and using both
calling handlers from the \R engine as well \Cpp exception forwarding might
lead to undetermined results. Future versions of \pkg{Rcpp} might attempt to
to improve this issue.

\subsection[R errors in C++]{\R errors in \Cpp}

\R itself currently does not offer \proglang{C}-level mechanisms to deal with errors. To
overcome this problem, \pkg{Rcpp} uses the \code{Rcpp::Evaluator}
class to evaluate an \R expression in an R-level \code{tryCatch}
block. The error, if any, that occurs while evaluating the
function is then translated into an \Cpp exception that can be dealt with using
regular \Cpp \code{try}/\code{catch} syntax.

An open (and rather hard) problem, however, is posed by the fact that calls
into the \proglang{C} API offered by \R cannot be reliably protected.  Such
...
particular, neither \Cpp class destructors nor \code{catch} parts of outer
...
situation via use of \Cpp exception handling, existing code calling
the into \R \proglang{C} API cannot be amended just by having an outer layer
...
rewrite the convolution example from `Writing R Extensions' \citep[Chapter 5]{R:Extensions}
...
and aims to provide R-like syntax in \proglang{C++}. While a fuller discussion of Rcpp sugar is
beyond the scope of this article, we have included
another version of the convolution algorithm based on Rcpp sugar for illustrative purposes here:
...
The first implementation, written in \proglang{C} and using the traditional \R API,
provides our base case. It takes advantage of pointer arithmetics and therefore
does not to pay the price of \Cpp object encapsulation or operator overloading.
...
%The slowest solution uses the more efficient new \pkg{Rcpp} API. While
%already several times faster than the preceding solution, it
...
This also shows that the use of \Cpp may not necessarily imply any performance
penalty. Further, \Cpp \code{iterators} can be used to achieve the performance
...
vectorization at the \Cpp level which is responsible for this particular speedup.
...
focuses on further improving interoperability between \R and \proglang{C++}.
Two core themes for on-going development are `Rcpp sugar' as well as `Rcpp modules', both of which are
also discussed in more detail in specific vignettes in the \pkg{Rcpp} package.

`Rcpp sugar' offers syntactic
sugar at the \Cpp level, including optimized binary operators and many
\R functions such as \code{ifelse}, \code{sapply}, \code{any}, \code{head},
\code{tail}, and more.
...
$s$. This was shown in Table~\ref{fig:rnormCode} in
Section~\ref{sec:functions} above where
the \R expression \code{rnorm(10L, sd=100)}
was rewritten in \Cpp as \code{rnorm(10, 0, 100)}. Note that
\Cpp semantics require the second parameter to be used here, which is
different from the \R case.

`Rcpp modules' allows programmers to expose \Cpp functions and classes at the
\R level. This offers access to \Cpp code from \R using even less interface
code than by writing accessor functions. Modules are inspired by the
\code{Boost.Python} library
\citep{Abrahams+Grosse-Kunstleve:2003:Boost.Python} which provides similar
functionality for \proglang{Python}. \Cpp Classes exposed by Rcpp modules are
shadowed by Reference Classes which have been introduced in \R 2.12.0.


\section{Summary}

The \pkg{Rcpp} package presented in this paper greatly simplifies integration of
compiled \Cpp code with \proglang{R}.
%
\pkg{Rcpp} provides a \Cpp class hierarchy which allows manipulation of \R data structures in \Cpp
...
Data interchange between \R and \Cpp code is performed by the \code{wrap()} and
\code{as()} template functions. They allow the programmer to write logic in terms
of \Cpp data structures, and facilitate use of modern libraries such as the
...
By using only thin wrappers around \code{SEXP} objects and adopting \Cpp
